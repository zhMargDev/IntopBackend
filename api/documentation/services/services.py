...
- Для фильтрации по другим параметрам: `GET /services/all?service_id=1&min_price=50&max_price=200&payment_method=credit_card`
...
- `lat`: (обязательный) Местоположение объявления.
- `lon`: (обязательный) Местоположение объявления.
...
- is_store`: (обязательный) Это магазин или нет
...
**Как можно отправить запрос**:
- `PUT /services/update/{service_id}`

**Какие параметры можно указать**:
- `service_id`: (обязательный) ID объявления, которое нужно обновить.
...
- `lat`: (необязательный) Новое местоположение объявления.
- `lon`: (необязательный) Новое местоположение объявления.
...
- `is_store`: (необязательный) Это магазин или нет.
- `picture`: (необязательный) Новое изображение объявления в формате файла.

**Пример запроса**:
```bash
curl -X PUT "http://localhost:8000/services/update/1" \
-H "Authorization: Bearer your_jwt_token" \
-F "user_id=1" \
...
**Как можно отправить запрос**:
- `DELETE /services/delete/{service_id}`

**Какие параметры можно указать**:
- `service_id`: (обязательный) ID объявления, которое нужно удалить.
- `user_id`: (обязательный) ID пользователя, который удаляет объявление. Должен совпадать с ID пользователя в токене.

**Пример запроса**:
```bash
curl -X DELETE "http://localhost:8000/services/delete/1" \
-H "Authorization: Bearer your_jwt_token" \
-F "user_id=1"
